...
class MultipleReplacementsError(Exception):
    ...
    def __init__(self, count: int):
        self.count = count
        super().__init__(f"The text in the SEARCH section was found multiple times in the resume. Make sure to include enough context lines to uniquely match the lines to change.")
...
class NoReplacementError(Exception):
    ...
    def __init__(self):
        super().__init__("Could not find text in the SEARCH section in the resume. Make sure it matches the contents of the resume exactly, including whitespace, comments, tags, etc.")
